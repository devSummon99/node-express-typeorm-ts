...
export const createTask = async (req: Request, res: Response) => {
    ...
    try {
        ...
        if (taskFind) res.status(400).json("Task has already been created");
        else {
            ...
            newTask.state = state;
            if (!user) { res.status(404).json("User no found"); }
...
export const updateTask = async (req: Request, res: Response) => {
    ...
    try {
        ...
        if (task) {
            ...
            res.status(404).json("Task no found");
        }
    } catch (err) {
        if (err instanceof Error)
            res.status(501).json(err.message);
...
export const deleteTask = async (req: Request, res: Response) => {
    ...
    try {
        ...
        if (task) {
            ...
            res.status(203).json("Task has been deleted");
        } else {
            res.status(404).json("Task no found");
        }
    } catch (err) {
